...
const CATEGORY_ID_PREFIX = "search-document-category";
const DOCUMENT_ID_PREFIX = "search-document-type";
...
function buildFilterRegistry(): FilterMeta[] {
  ...
  for (const option of searchDocumentCategories) {
    items.push({
  ...
  for (const option of searchDocumentTypes) {
    items.push({
...
function buildInitialChecked(): Record<string, boolean> {
  ...
  /* Figma 6571:104998 — default active filters */
  checked[`${CATEGORY_ID_PREFIX}-mccb-susol-ul`] = true;
  checked[`${DOCUMENT_ID_PREFIX}-catalogs`] = true;
  checked[`${DOCUMENT_ID_PREFIX}-manuals`] = true;
  syncCategoryParentState(checked, `${CATEGORY_ID_PREFIX}-lv-products`);

  return checked;
}

type SearchDocumentsFilterContextValue = {
...
const SearchDocumentsFilterContext =
  createContext<SearchDocumentsFilterContextValue | null>(null);

export function useSearchDocumentsFilter() {
  const context = useContext(SearchDocumentsFilterContext);

  if (!context) {
    throw new Error(
      "useSearchDocumentsFilter must be used within SearchDocumentsFilterProvider",
    );
  }
...
export function SearchDocumentsFilterProvider({
  ...
  return (
    <SearchDocumentsFilterContext.Provider value={value}>
      {children}
    </SearchDocumentsFilterContext.Provider>
  );
}
