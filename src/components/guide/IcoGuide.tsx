  ...
  return (
    <section className="guide-doc ico-guide">
      <GuidePageHeader
        ...
        description={
          <>
            <code>public/ico</code> SVG ({icoItems.length}개). 앱:{" "}
            <code>/ico/파일명.svg</code>. 상세: <code>docs/ICON_GUIDE.md</code>.
            GNB: <a href="/guide/gnb">GNB Guide</a> · <code>docs/GNB_GUIDE.md</code>
          </>
        }
      />
      ...
      <div className="guide-doc__rules ico-guide-rules">
        ...
        <ul>
          <li>
            모든 파일은 <code>ico_</code> 접두사를 사용합니다.
          </li>
          <li>
          ...
          <li>
            ...
            Downloads 필터 <code>ico_check_block</code> /{" "}
            <code>ico_checked_black</code> —{" "}
            <a href="/guide/components#check">Component Guide · Check</a>
          </li>
          <li>
            CSS 유틸(<code>icon_arrow-14</code> 등)은{" "}
            <code>globals.css</code>에서 SVG를 background로 매핑합니다.
          </li>
        </ul>
      </div>
      ...
      {icoCategoryOrder.map((category) => {
        const items = getIcoItemsByCategory(category);
        ...
        return (
          <section
            key={category}
            id={`ico-${category}`}
            className="guide-doc__section ico-guide-section"
          >
            <h2 className="guide-doc__section-title ico-guide-section-title">
              {icoCategoryLabels[category]}
              <span className="guide-doc__count ico-guide-section-count">
                {items.length}
              </span>
            </h2>
            ...
            <div className="ico-guide-grid">
              {items.map((item) => (
                <article key={item.id} className="ico-card">
