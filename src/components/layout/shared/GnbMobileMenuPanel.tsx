...
import { gnbNavItems } from "@/data/gnb";
import {
  findGnbNavItem,
...
export default function GnbMobileMenuPanel({
  ...
  if (level === "depth4" && activeNavId && activeDepth2Id && activeDepth3Id) {
    ...
    if (depth4Data) {
      return (
        <GnbMobileDepth4Menu
          data={depth4Data}
          backLabel={getMobileDepth4BackLabel(activeNavId, activeDepth2Id)}
  ...
  if (level === "depth3" && activeNavId && activeDepth2Id) {
    const depth3Items = getMobileDepth3Items(activeNavId, activeDepth2Id);

    return (
      <GnbMobileDepth3Menu
        items={depth3Items}
        backLabel={getMobileDepth3BackLabel(activeNavId)}
  ...
  if (level === "depth2" && activeNavId) {
    const navItem = findGnbNavItem(activeNavId);
