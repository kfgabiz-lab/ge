...
| `main_notic` | `MainVisual.tsx` | `main_visual` 하단 · MO Figma 6571:92270 · dark navy 123px · 15/23 Medium (가이드 미등록) |
...
| `common_404_search` | `NotFoundSearch.tsx` | `common-404.css` · 검색 + Popular Keywords |
| `common_404_links` | `NotFoundHelpfulLinks.tsx` | `common-404.css` · Helpful Links 4카드 |
| `cookie_settings_modal` | `CookieSettingsModal.tsx` | `globals.css` · P-FO-COMMON-020000P · 우하단 동의 배너 |
| `cookie_preferences_modal` | `CookiePreferencesModal.tsx` | `globals.css` · P-FO-COMMON-040000M · 상세 분류 설정 |
...
| `common_404_links` | 상단 120px · 하단 180px · 4열 카드 · 아이콘 80px |

아이콘: `ico_404_home_80` · `ico_404_download_80` · `ico_404_contact_80` · `ico_404_request_80`
...
모바일 Figma [7334:130901](https://www.figma.com/design/FJa9pa866Be2aj5HYV717D/LSEA_%EB%94%94%EC%9E%90%EC%9D%B8?node-id=7334-130901) — 패널 너비 `335px` · viewport 상하 `61px` · 좌우 inset `24px` · 제목 20/30 · 본문 15/23 · 항목 제목 18/26 · 체크 간격 `10px` · 항목 간격 `20px` · 패널 내부 스크롤 · 하단 버튼 미노출.
...
| 순서 | 클래스 | 컴포넌트 | id · 비고 |
|------|--------|----------|-----------|
| 1 | `support_service_training_session_detail` | `EngineeringTrainingSessionDetail.tsx` | `engineering-training-session-detail` · 탭 · 아젠다 · 등록 · 사이드바 `__aside--pc` / `__aside--mo` |

데이터: `engineeringTrainingSessionDetailContent.ts` · breadcrumb: Services > Training > Engineering Training > curriculum > session date

| 헬퍼 (섹션 루트 아님) | 컴포넌트 | 비고 |
|----------------------|----------|------|
| `EngineeringTrainingSessionCountdown` | countdown | Time Remaining (정적 UI · PC `session-countdown-bg.svg` · MO `session-countdown-bg-mo.svg`) |
...
| `support_service_training_session_detail__table` | agenda | No · Time · Contents · Trainer · mobile swipe (`SessionDetailTableScroll`, Figma 6880:146799) |
