...
3-1. **정적 자산(public) 동반 확인** — 이관 파일(및 관련 CSS)이 참조하는 이미지/아이콘 경로(`/ico/...`, `/img/...` 등)를 코드에서 추출해, 해당 파일이 `ls-publish/public/`에 있고 `fo/public/`에 없으면 함께 복사한다. 코드만 옮기고 자산은 누락되는 사고를 방지하기 위함(2026-07-14 contact-us 이관 시 아이콘 6개·푸터 배경 2개 누락 사례).
...
- **"재사용(신규 생성 안 함)"으로 표시한 기존 컴포넌트 목록** — fo에 이미 존재해 새로 만들지 않고 그대로 참조한 컴포넌트. `fo-qa-validator`가 이 목록을 검증 대상에 포함해, 재사용 컴포넌트가 요구하는 자산(이미지/CSS 등)까지 실제로 정상 이관돼 있는지 확인한다
- 이관 중 발견한 이슈/질문 사항
