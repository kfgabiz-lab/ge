...
`docs/ge_guide/builder/00-4.builder_agent_common_principles.md` — 아래 "패턴 카탈로그"는 이 문서의
원칙을 실제 코드로 어떻게 변환하는지 보여주는 예시다. 새 패턴이 필요하면 00-4의 원칙부터 확인한다.
...
4. **주석 작성 금지** — 새 컴포넌트/공통 함수도 사용법 주석 없이 이름만으로 의도가 드러나게 작성
