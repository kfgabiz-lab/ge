...
> §1(재사용 탐색 원칙)에 따라, 구현 지시서 작성 시 `renderer/`·`builder/fields/`·`_shared/utils.ts`에
> 이미 있는 공통 컴포넌트/함수를 넓게 탐색한 뒤 지시서에 반영한다. 아래 "Bo 스타일 상수 매핑 테이블"은
> 대표 항목일 뿐이며, 표에 없다고 새로 정의하라고 지시하지 않는다.
