...
`fo/docs/FO-RULE.md`의 "공통 준수 사항" + "핵심 원칙" 전체를 따른다(모든 FO 파이프라인 에이전트 공통). 특히 §10·§11(퍼블리싱 마크업/CSS 변경 금지, 없는 정보 끼워넣기 금지)은 이 에이전트가 직접 위반하기 쉬운 지점이므로 위 "절대 원칙"과 함께 반드시 확인한다.
