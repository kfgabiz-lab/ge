...
- **⚠️ 주석을 작성하지 않는다.** 설명용이든 WHY(왜 이렇게 했는지)용이든 어떤 형태의 주석도 코드에 추가하지 않는다. 기존 코드를 수정할 때 남아있는 주석을 발견하면 함께 제거한다.
...
- **⚠️ 소스코드에 임시로 만들어 넣은 데이터를 남기지 않는다.** 바인딩 확인용으로 넣은 임시 배열·목업 값·테스트용 하드코딩은 완료 보고 전 반드시 제거한다.
- **⚠️⚠️ 퍼블리싱(ls-publish)에 없는 HTML/CSS를 새로 작성하지 않는다.** 위 "퍼블리싱된 마크업/CSS 절대 변경 금지" 원칙의 연장 — 마크업을 안 바꾸는 것뿐 아니라, 애초에 pub에 없는 새 태그/클래스/스타일을 만들어 추가하는 것도 동일하게 금지된다.
- **⚠️ 기획서에 없는 내용을 추측하거나 확대해석하지 않는다.**
- **⚠️ 기획서에 없는 validation을 추가하지 않는다.** 이메일 형식 체크, 필수값 체크, 길이 제한 등 기획서/설계 문서에 명시 안 된 클라이언트 유효성 검사를 임의로 붙이지 않는다.
- **⚠️ 기획서에 없는 로직을 추가하지 않는다.** 데이터 가공, 자동 폴백, 조건 분기 등 문서에 없는 동작을 임의로 만들지 않는다.
- **기획 내용에 반하는 역기획 제안을 하지 않는다.** 승인된 설계와 다른 방식이 "더 낫다"고 임의로 바꿔 구현하지 않는다.
- **⚠️ 기획서에 없는 폴백과 데드코드를 남기지 않는다.** 하드코딩된 대체 문구/기본값을 스펙 없이 추가하지 않고, 개발 중 더 이상 참조되지 않게 된 기존 상수·함수를 발견하면 정리 대상으로 보고한다.
