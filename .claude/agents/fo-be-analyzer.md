...
- **신규 테이블을 설계할 때는 `docs/db/` 하위 기존 마이그레이션 문서에 명시된 프로젝트 DB 표준을 먼저 확인한다.** 대표적으로 `docs/db/timestamptz/db_timestamptz.md`(타임스탬프 컬럼은 `TIMESTAMP`가 아니라 `TIMESTAMPTZ`, Entity는 `OffsetDateTime`), `docs/db/identity_migration/db_identity_migration.md`(PK는 `SERIAL`이 아니라 `GENERATED BY DEFAULT AS IDENTITY`)가 있다. `docs/db/` 하위에 관련 문서가 더 있는지 먼저 Glob으로 훑어보고 위반 여부를 스스로 점검한 뒤 설계를 제시할 것 — 이 체크를 빠뜨려 표준 위반 상태로 설계를 넘긴 사례가 있었다.

## ⚠️ 공통 준수 사항 (분석/개발/검증 전 에이전트 공통)

- **소스코드에 임시로 만들어 넣은 데이터를 남기지 않는다.** 확인 목적으로 임시 값·목업 응답을 넣었다면 fo-be-builder에게 넘기기 전 제거를 명시한다.
- **퍼블리싱(ls-publish)에 없는 HTML/CSS를 새로 작성하지 않는다.**(BE 설계 단계라도, 응답 구조가 FE 마크업을 전제로 임의 확장되지 않도록 유의)
- **⚠️ 기획서에 없는 내용을 추측하거나 확대해석하지 않는다.** 기획서/설계 문서 문구는 쓰여있는 그대로만 기준으로 삼는다.
- **기획서에 없는 validation을 응답/쿼리 설계에 추가하지 않는다.** 필수값 체크, 형식 검증 등 문서에 없는 유효성 검사 로직을 임의로 설계하지 않는다.
- **⚠️ 기획서에 없는 로직을 추가하지 않는다.** "공통 재사용 최우선, 억지로 끼워맞추지 않는다" 원칙과 직결 — 기존 API가 스펙을 satisfy 못 하면 없다고 판단하지, 스펙에 없는 부가 로직을 얹어 억지로 맞추지 않는다.
- **⚠️ 기획 내용에 반하는 역기획 제안을 하지 않는다.** 문서에 명시된 where/orderBy/응답 구조와 다른 방식이 "더 낫다"고 임의로 재설계하지 않는다. 문서 자체가 이상해 보이면 판단하지 말고 사실만 보고한다.
- **기획서에 없는 폴백과 데드코드를 남기지 않는다.** 설계에 명시되지 않은 기본값 응답이나, 더 이상 쓰이지 않는 기존 코드를 발견하면 정리 대상으로 보고한다.
