...
- **data-slug 값 자체를 후보로 제시하거나 임의로 짓지 않는다** — slug는 bo Slug 레지스트리에 이미 등록돼 있거나 사용자가 신규로 지정하는 값이며, 정할 권한은 사용자에게 있다

## 공통 준수 사항

`fo/docs/FO-RULE.md`의 "공통 준수 사항" + "핵심 원칙" 전체를 따른다(모든 FO 파이프라인 에이전트 공통).

## ⚠️⚠️ 퍼블리싱된 마크업/CSS 변경 절대 금지

이 에이전트가 하는 일은 기존 태그에 `data-slug`/`data-slug-repeat`/`data-slug-item`/`data-slugKey`/`data-slugKey-attr` **속성만 추가**하는 것이다.

- ❌ 새 태그(div/span 등) 생성 금지
- ❌ 기존 태그 구조 변경 금지
- ❌ className/style/CSS 수정 금지
- 마크업 구조가 데이터 바인딩에 부족해 보여도 임의로 태그를 추가하거나 스타일을 고치지 말고, 그 즉시 작업을 멈추고 사용자에게 질문한다.
- ❌ **퍼블리싱에 없는 정보를 억지로 끼워넣지 않는다** — 퍼블리싱 결과물에 없는 텍스트/이미지/필드를 데이터 바인딩을 위해 새로 만들어 추가하지 않는다. 데이터 항목이 더 필요해 보여도 임의로 지어내지 말고, 그 즉시 사용자에게 확인한다.
...
- `data-slug` **값 자체는 에이전트가 짓지 않는다.** bo Slug 레지스트리에 이미 있는 값을 쓸지, 사용자가 신규로 지정할 값을 쓸지는 사용자만 정한다 — 사용자가 값을 알려줄 때까지 `data-slug="TODO"`로 비워둔다. 에이전트가 할 일은 여기가 데이터 자리라는 **구조**(`data-slug-repeat`, `data-slug-item`, `data-slugKey`, `data-slugKey-attr`)를 태깅하는 것까지다
