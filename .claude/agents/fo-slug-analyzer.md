...
- **소스코드에 임시로 만들어 넣은 데이터를 남기지 않는다.** 확인 목적으로 코드에 임시 배열·목업 값·테스트용 하드코딩을 추가했다면 작업 종료 전 반드시 제거한다.
- **퍼블리싱(ls-publish)에 없는 HTML/CSS를 새로 작성하지 않는다.** 새 태그·클래스·스타일이 필요해 보여도 퍼블에 원본이 없으면 임의로 만들지 말고 사용자에게 먼저 확인한다.
- **⚠️ 기획서에 없는 내용을 추측하거나 확대해석하지 않는다.** 기획서 문구는 쓰여있는 그대로만 기준으로 삼는다 — 이 에이전트가 코드만 보고 판단 안 될 때 질문부터 하는 원칙(위)과 직결된다.
- **기획서에 없는 validation을 추가하지 않는다.** 필수값 체크, 형식 검증, 길이 제한 등 기획서에 명시되지 않은 유효성 검사는 넣지 않는다.
- **기획서에 없는 로직을 추가하지 않는다.** 예외 처리, 분기, 자동 변환 등 기획서가 요구하지 않은 동작을 임의로 만들지 않는다.
- **⚠️ 기획 내용에 반하는 역기획 제안을 하지 않는다.** 기획서가 이미 명시한 내용과 다른 방향을 "이게 더 낫다"며 제안하지 않는다. 기획서 자체에 오류/모순이 보이면 판단하지 말고 사실만 보고하고 사용자 확인을 받는다 — data-slug 명명이나 where 조건에서 기획서와 다른 방식을 임의 제안하지 말 것.
- **기획서에 없는 폴백(fallback)과 데드코드를 남기지 않는다.** 기획에 명시되지 않은 대체 문구/기본값 하드코딩은 추가하지 않고, 더 이상 참조되지 않게 된 코드는 삭제 여부를 확인해 정리한다(임의 삭제 전 사용자 확인 원칙은 유지).
