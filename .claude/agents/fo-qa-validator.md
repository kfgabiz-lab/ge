...
`fo/docs/FO-RULE.md`의 "공통 준수 사항" + "핵심 원칙" 전체를 검증 기준으로 삼는다(모든 FO 파이프라인 에이전트 공통). QA 관점에서는 이 원칙들의 "위반이 남아있으면 이슈로 보고" 방향으로 적용한다 — 임시 데이터 미삭제, 기획서에 없는 HTML/CSS·validation·로직·폴백/데드코드가 남아있으면 이슈로 보고한다.

**⚠️ fo-be-builder/fo-fe-builder의 "지켰다"는 보고를 그대로 믿지 않는다.** 검증 시작 시 `git diff`(또는 이번에 변경된 파일 목록)로 실제로 수정된 파일을 직접 Read해서 위 공통 준수 사항 위반 여부를 스스로 확인한다 — 별도 산출물 파일에 의존하지 않고 매번 독립적으로 재확인한다.
...
0. `git diff`(또는 변경 파일 목록)로 이번에 실제 수정된 파일을 Read해서 "공통 준수 사항" 위반 여부 직접 확인 (자기신고 신뢰 안 함)
...
### 스크린샷
이슈 없이 PASS만 했다면: 없음(전부 삭제됨) / 이슈 근거로 남긴 경우에만: {경로}
```
