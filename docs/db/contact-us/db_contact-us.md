...
erDiagram
    CONTACT_US_INQUIRY {
        ...
        VARCHAR(255) product_category_lv1 "NULL - 제품 카테고리 Lv1(라벨)"
        VARCHAR(255) product_category_lv2 "NULL - 제품 카테고리 Lv2(라벨)"
        VARCHAR(255) product_category_lv3 "NULL - 제품 카테고리 Lv3(라벨)"
        VARCHAR(20) product_category_lv1_id "NULL - 제품 카테고리 Lv1 코드(예: L06)"
        VARCHAR(20) product_category_lv2_id "NULL - 제품 카테고리 Lv2 코드(예: L06-01)"
...
| `product_category_lv1` | VARCHAR(255) | YES | NULL | 제품 카테고리 1레벨 | Product Category Lv1 |
| `product_category_lv2` | VARCHAR(255) | YES | NULL | 제품 카테고리 2레벨 | Product Category Lv2 |
| `product_category_lv3` | VARCHAR(255) | YES | NULL | 제품 카테고리 3레벨(라벨) | Product Category Lv3 |
| `product_category_lv1_id` | VARCHAR(20) | YES | NULL | 제품 카테고리 Lv1 코드(예: `L06`) | 카테고리 선택 시 코드값 |
| `product_category_lv2_id` | VARCHAR(20) | YES | NULL | 제품 카테고리 Lv2 코드(예: `L06-01`) | 카테고리 선택 시 코드값 |
...
> **product_category_lv1_id / lv2_id (코드값)** 은 향후 BO 관리자 화면에서 코드 기반 필터링을 대비해 함께 저장한다(사용자 승인, 2026-07-21). FO 폼이 카테고리 선택 시 함께 보유한 코드값을 그대로 전달한다.
...
| `product_category_lv1~3` nullable | ✅ | FO 폼상 카테고리는 필수 표시(*) 없음 — 미선택 허용 |
| `product_category_lv1_id`/`lv2_id` 추가 저장 | ✅ | BO 관리자 화면(추후)에서 코드 기반 필터가 필요할 수 있어 라벨과 함께 저장(사용자 승인) — Lv3는 코드 체계가 없어 라벨만 유지 |
...
CREATE TABLE contact_us_inquiry (
    ...
    product_category_lv1  VARCHAR(255),
    product_category_lv2  VARCHAR(255),
    product_category_lv3  VARCHAR(255),
    product_category_lv1_id VARCHAR(20),
    product_category_lv2_id VARCHAR(20),
