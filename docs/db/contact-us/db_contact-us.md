...
erDiagram
    CONTACT_US_INQUIRY {
        ...
        BIGINT product_category_lv1_id "NULL - 제품 카테고리 Lv1 (devices-tree/category-data 행 rowId)"
        BIGINT product_category_lv2_id "NULL - 제품 카테고리 Lv2 (devices-tree/category-data 행 rowId)"
        BIGINT product_category_lv3_id "NULL - 제품 카테고리 Lv3/제품 (devices-tree/category-data 행 rowId)"
...
| `product_category_lv1_id` | BIGINT | YES | NULL | 제품 카테고리 Lv1 — devices-tree(`GET /api/v1/fo/gnb/devices-tree`) 응답 행의 rowId(=category-data page_data PK) | Product Category Lv1 |
| `product_category_lv2_id` | BIGINT | YES | NULL | 제품 카테고리 Lv2 — 위와 동일, depth2 행 rowId | Product Category Lv2 |
| `product_category_lv3_id` | BIGINT | YES | NULL | 제품 카테고리 Lv3(제품) — 위와 동일, depth3 행 rowId | Product Category Lv3 |
...
> **product_category_lv1_id / lv2_id / lv3_id** 는 devices-tree(GNB 메가메뉴와 동일 엔드포인트, `PageDataService.findDevicesTree`)가 반환하는 flat 행의 rowId를 그대로 저장한다(BIGINT). 라벨 텍스트는 저장하지 않고, 필요하면 저장된 rowId로 devices-tree를 다시 조회해 라벨을 역조회한다(2026-07-22 확정 — 최초엔 "L06" 스타일 카테고리 코드값을 저장하는 것으로 설계했으나, 그런 코드 체계가 실제로는 어디에도 존재하지 않아 rowId 기준으로 수정).
...
| `product_category_lv1_id~lv3_id` nullable | ✅ | FO 폼상 카테고리는 필수 표시(*) 없음 — 미선택 허용 |
| `product_category_lv1_id~lv3_id` 타입 BIGINT(rowId), 라벨 컬럼 없음 | ✅ (수정) | 최초엔 라벨(VARCHAR) + Lv1/Lv2 코드(VARCHAR "L06" 스타일)로 설계했으나, 그런 카테고리 코드 체계가 실제 데이터 어디에도 없음이 확인되어 devices-tree(category-data) 행의 rowId(BIGINT)를 그대로 저장하는 구조로 변경. Lv3(제품)도 동일하게 rowId 저장(기존엔 라벨만 저장, Id 없음 — 수정) |
...
CREATE TABLE contact_us_inquiry (
    ...
    product_category_lv1_id BIGINT,
    product_category_lv2_id BIGINT,
    product_category_lv3_id BIGINT,
...
- **COUNTRY**(범용, 다른 화면도 재사용): ISO 3166-1 alpha-2 **대문자**(`US`/`CA`/`KR`)로 통일. 기존 FO 폼은 소문자 `us`를 하드코딩했으나, API 전환 후 FE는 API가 내려주는 `code`를 그대로 전송하므로 대문자 캐논으로 고정(대소문자 혼용 검증실패 방지).
...
('COUNTRY',      '국가',     'ISO 3166-1 alpha-2 국가코드(범용)', TRUE, 'system', 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
-- 3) 코드 상세 등록 — COUNTRY (초기 3개, 나머지는 BO 코드관리에서 추가)
INSERT INTO code_detail (group_id, code, name, sort_order, is_active, created_by, updated_by, created_at, updated_at) VALUES
((SELECT id FROM code_group WHERE group_code = 'COUNTRY'), 'US', 'United States', 1, TRUE, 'system', 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
((SELECT id FROM code_group WHERE group_code = 'COUNTRY'), 'CA', 'Canada',        2, TRUE, 'system', 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
((SELECT id FROM code_group WHERE group_code = 'COUNTRY'), 'KR', 'Korea',         3, TRUE, 'system', 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
```
