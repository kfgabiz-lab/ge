...
집계: **Critical 1건 / Warning 17건(B-4·C-3 수정완료 2건 포함) / Info 11건 / UNPROVEN 0건**
...
| ✅ **C-3** | `bo-api\...\common\client\ExternalApiClient.java:79, 93-94` + `application-developer.yml:67` | 3. JWT/보안설정 | C | **로그 파일에 reCAPTCHA 시크릿과 CTP client_secret이 평문으로 남는다.** 79행이 요청 body 전체를, 93~94행이 응답 body 전체를 DEBUG로 출력. developer 프로파일은 `com.ge.bo: DEBUG`이며 logback의 `<springProfile name="dev,!local">` 분기로 `C:/app/logs/bo-api.log`에 기록되는데, **마스킹 컨버터(`%maskedMsg`)는 local 프로파일에만 적용**된다(logback-spring.xml:13 vs 58). 결과적으로 `secret=<reCAPTCHA 시크릿>`, `client_secret`, `access_token`이 파일에 쌓인다. — ✅ **수정 완료(2026-08-10)**: 마스킹 규칙을 로그 출력 계층(`%maskedMsg`)에서 **애플리케이션 소스 계층**으로 끌어올려, 어떤 프로파일·어떤 어펜더(PatternLayout/JsonEncoder)를 타든 시크릿이 로거에 넘어가기 전에 치환되도록 했다. ① 신규 `com.ge.bo.logging.SensitiveDataMasker`로 치환 규칙 단일화(대상에 `token` 추가 → `access_token`/`accessToken` 커버) → `MaskingConverter`는 이 유틸에 위임. ② `ExternalApiClient`의 요청/응답 DEBUG 2건 + 4xx/5xx/네트워크/기타 예외 WARN·ERROR 4건 전부 url·body를 `SensitiveDataMasker.mask()` 경유로 출력(DEBUG는 `isDebugEnabled()` 가드로 비활성 시 마스킹 비용 0). ③ `logback-spring.xml:58`(`dev,!local` FILE 어펜더)의 `%msg` → `%maskedMsg`로 교체해 `ExternalApiClient` 외 로거까지 파일 기록분은 2차 방어. **실측 검증**(profile=`developer`, `com.ge.bo: DEBUG` — 즉 취약 조건 그대로 재기동 후 실제 HTTP 호출): (1) `POST /api/v1/auth/login`(recaptchaToken=`test-token`)으로 Google siteverify 실호출 → 파일 로그 `body={secret=****], response=[test-token]}` + 응답 `success=true`(=실제 요청에는 진짜 시크릿이 그대로 실려 나갔고 로그만 가려짐, 기능 무회귀). (2) `POST /api/v1/public/contact-us/answer`로 Salesforce OAuth 토큰 발급 실호출 → 요청 `client_secret=****]`, 응답 `CtpTokenResponse[accessToken=****, instanceUrl=…, tokenType=Bearer]`, status=200(토큰 발급 성공) + 후속 `IF_SRR_NAHP_CTP_0002` 호출도 200 정상. (3) 재기동 이후 구간에서 평문 잔존 검색: 파일 로그 `client_secret` 실제값 0건, 콘솔(JSON stdout) `client_secret` 0건 / reCAPTCHA 시크릿 0건 — 콘솔 JSON은 `arguments` 배열에 마스킹된 문자열이 들어가 있어 JsonEncoder 경로도 차단 확인. (4) 회귀 확인: 실제 계정 `comlbg` 로그인 200 성공. **잔여**: ⓐ 기존 `C:/app/logs/bo-api.log`에 수정 전 평문 시크릿 33건이 그대로 남아 있어 **기존 로그 파기 + reCAPTCHA/CTP 시크릿 교체 필요**(코드 수정으로 소급 제거 불가). ⓑ 같은 셀의 F-2(테스트 시크릿 하드코딩)가 미해결이라 매트릭스 3행 C열은 FAIL 유지. ⓒ `CtpFileDownloadService.java:37`이 CTP 함수키(`code=`)를 URL 쿼리에 실어 보내는데 이 키 이름은 마스킹 대상이 아니다 — C-3 범위 밖이라 미조치, 별도 항목으로 판단 필요. |
...
| 4 | **D-1** developer 기본 프로파일의 TLS 검증 해제 (C-3 시크릿 DEBUG 로깅은 ✅ 수정완료, 기존 로그 파기·시크릿 교체만 잔여) | 기본값이 위험한 쪽이라 무설정 배포 시 자동 노출 |
